...
\begin{algorithmic}[1]
    ...
    \Procedure{LearnSplit}{$\mathcal{D}, \theta, E, \lambda, \beta, \sigma$}
        \State Initialize $\Theta = \{\boldsymbol{\mu}_w, \boldsymbol{\Sigma}_w, \mu_b, \sigma_b^2\}$ \Comment{Initialize variational parameters}
...
The evaluation scores on the regression datasets are presented in Table~\ref{tab:regression_results}.
...
\begin{tabular}{@{}lccc@{}}
\toprule
Dataset & SPYCT-single tree & SPYCT-ensemble & VSPYCT \\ \midrule
...
puma8NH         & 2.80                            & \textbf{2.62}                        & 2.74                \\ \bottomrule
\end{tabular}
\end{table}
...
balance               & 0.60                            & 0.66                        & \textbf{0.73}                \\ \bottomrule
\end{tabular}
\end{table}

The performance of the models on the classification datasets is shown in Table~\ref{tab:classification_results}.
